...
static int32_t mndProcessConfigDnodeReq(SRpcMsg *pReq) {
  ...
  if (strcasecmp(cfgReq.config, "resetlog") == 0) {
    ...
    pItem = cfgGetItem(pCfg, dcfgReq.config);
    if (!pItem || (pItem->dynScope & CFG_DYN_SERVER) == 0) {
      mError("dnode:%d, failed to config:%s, not support", cfgReq.dnodeId, dcfgReq.config);
      terrno = TSDB_CODE_INVALID_CFG;
      goto _err_out;
    }

    if (cfgItemCheckRange(pItem, dcfgReq.value) != 0) goto _err_out;
  }
