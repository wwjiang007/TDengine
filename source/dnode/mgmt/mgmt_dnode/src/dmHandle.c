...
static void dmMayShouldUpdateAnalFunc(SDnodeMgmt *pMgmt, int64_t newVer) {
  ...
  if (contLen < 0) {
    dError("failed to serialize analysis function request since %s", tstrerror(contLen));
    return;
  }
  ...
  code = rpcSendRequest(pMgmt->msgCb.clientRpc, &epset, &rpcMsg, NULL);
  if (code != 0) {
    dError("failed to send retrieve analysis function request since %s", tstrerror(code));
  }
}
